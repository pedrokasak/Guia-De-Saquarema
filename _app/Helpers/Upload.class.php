...
class Upload {
    ...
    public function File(array $File, $Name = null, $Folder = null, $MaxFileSize = null) {
        ...
        $MaxFileSize = ( (int) $MaxFileSize ? $MaxFileSize : 8 );

        $FileAccept = [
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
            'application/pdf','application/powerpoint','application/mspowerpoint','text/plain'
        ];

        if ($this->File['size'] > ($MaxFileSize * (1024 * 1024))):
            $this->Result = false;
            WSErro("Oopsss :( Arquivo muito grande, tamanho máximo permitido de {$MaxFileSize}mb", WS_ERROR);
            //$this->Error = "Arquivo muito grande, tamanho máximo permitido de {$MaxFileSize}mb";
        elseif (!in_array($this->File['type'], $FileAccept)):
            $this->Result = false;
            $this->Error = 'Tipo de arquivo não suportado. Envie .PDF, .DOCX, .PPT, .TXT!';
        else:
            $this->CheckFolder($this->Folder);
            $this->setFileName();
            $this->MoveFile();
    ...
    public function Media(array $Media, $Name = null, $Folder = null, $MaxFileSize = null) {
        ...
        elseif (!in_array($this->File['type'], $FileAccept)):
            $this->Result = false;
            WSErro("Tipo de arquivo não suportado. Envie audio MP3 ou vídeo MP4 ou MOV!", WS_ERROR);
            //$this->Error = 'Tipo de arquivo não suportado. Envie audio MP3 ou vídeo MP4 ou MOV!';
        else:
            $this->CheckFolder($this->Folder);
            $this->setFileName();
            $this->MoveFile();
    ...
    private function UploadImage() {
        ...
        if (!$this->Image):
            $this->Result = false;
            $this->Error = 'Tipo de arquivo inválido, envie imagens JPG,PNG, BITMAP ou GIF!';
        else:
            ...
            switch ($this->File['type']):
                ...
                case 'image/x-png':
                    ...
                    break;
                case 'image/bmp';
                    image2wbmp($NewImage, self::$BaseDir . $this->Send . $this->Name);
                    break;
                case 'image/gif';
    ...
    private function MoveFile() {
        if (move_uploaded_file($this->File['tmp_name'], self::$BaseDir . $this->Send . $this->Name)):
            ...
            $this->Result = false;
            WSErro("Erro ao mover o arquivo. Favor tente mais tarde!", WS_ERROR);
