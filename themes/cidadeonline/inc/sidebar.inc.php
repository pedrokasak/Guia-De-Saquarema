...
<aside class="main-sidebar">
    ...
    <section class="widget art-list last-publish">
        ...
        <?php
//            <!--120x80-->
        $side->ExeRead("ws_posts", "WHERE post_status = 1 AND post_id != :side ORDER BY post_date DESC LIMIT 4", "side={$post_id}");
        if ($side->getResult()):
    ...
    <section class="widget art-list most-view">
        ...
        <?php
        $side->ExeRead("ws_posts", "WHERE post_status = 1 AND post_id != :side ORDER BY post_views DESC LIMIT 4", "side={$post_id}");
        if ($side->getResult()):
